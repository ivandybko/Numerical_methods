...
std::vector<std::vector<T>> adams_bashforth(
	...
	auto solution = runge_kutta4(func, t_start, initial_conditions, t_start+3*tau, tau, false);
	...
	for(int i = 3; i < points_number-1; i++)
	{
		...
		for(int j = 0; j < equations_number; j++)
		{
			...
			u_n1[j]=solution[j+1][i-1];
			u_n2[j]=solution[j+1][i-2];
			u_n3[j]=solution[j+1][i-3];

		}
		for(int j = 0; j < equations_number; j++)
		{
			solution[j+1][i + 1] = solution[j+1][i] + tau/24.0*(55.0* func[j](t, u_n)-59.0*func[j](t - tau, u_n1)
		...
		if ( predictor_corrector )
		{
			...
			for(int j = 0; j < equations_number; j++)
			{
				u_cor[j]=solution[j+1][i+1];
			}
			for(int j = 0; j < equations_number; j++)
			{
				solution[j+1][i + 1] = solution[j+1][i] + tau/24.0*(9.0* func[j](t+tau, u_cor)+19.0*func[j](t, u_n)
